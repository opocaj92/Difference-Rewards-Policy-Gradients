...
class PredatorPrey:
    def __init__(self, n_agents = 3,
        ...
        self._preys_pos = None
        self._agents_pos = None

    def step(self, actions, preys_actions = None):
        actions = [int(a) for a in actions]
        preys_actions = [int(a) for a in preys_actions] if preys_actions is not None else None
        info = {
            "all_rewards": []
        }
        if self.return_all_rewards:
            original_agents = copy.copy(self._agents_pos)
            original_preys = copy.copy(self._preys_pos)
            for i in range(self.n_agents):
                ...
                for j in range(self.n_actions):
                    ...
                    self._take_preys_random_action()
                    info["all_rewards"][i].append(self._get_agents_reward())
                    self._agents_pos = copy.copy(original_agents)
                    self._preys_pos = copy.copy(original_preys)
        self._take_agents_action(actions)
        self._take_preys_random_action() if preys_actions is None else self._take_preys_selected_action(preys_actions)
    ...
    def get_agent_obs(self, agent_id):
        agent_obs = []
        for j, q in enumerate(np.concatenate((self._agents_pos, self._preys_pos), axis = 0)):
            if agent_id != j:
    ...
    def get_obs_size(self):
        return 2 * (self.n_agents + self.n_preys - 1) + self.n_actions * self.obs_last_action + 1 * self.obs_timestep_number
    ...
    def get_state_size(self):
        return (2 * (self.n_agents + self.n_preys - 1)) * self.n_agents + self.n_actions * self.n_agents * self.state_last_action + 1 * self.state_timestep_number
    ...
    def reset(self):
        self._n_steps = 0
        self.last_action = np.zeros((self.n_agents, self.n_actions))
        pos = np.random.choice(self.grid_size ** 2, size = self.n_agents + self.n_preys, replace = False)
        self._preys_pos = np.array([[p // self.grid_size, p % self.grid_size] for p in pos[:self.n_preys]])
    ...
    def render(self, mode = "human", close = False):
        ...
        for p in self._agents_pos:
            grid[p[0] * self.grid_size + p[1]] = np.array([64, 217, 64])
            if p[1] + 1 < self.grid_size:
                grid[p[0] * self.grid_size + p[1] + 1] = np.array([64, 217, 64])
            if p[1] - 1 >= 0:
                grid[p[0] * self.grid_size + p[1] - 1] = np.array([64, 217, 64])
            if p[0] + 1 < self.grid_size:
                grid[(p[0] + 1) * self.grid_size + p[1]] = np.array([64, 217, 64])
            if p[0]  - 1 >= 0:
                grid[(p[0] - 1) * self.grid_size + p[1]] = np.array([64, 217, 64])
            if p[0] + 1 < self.grid_size and p[1] + 1 < self.grid_size:
                grid[(p[0] + 1) * self.grid_size + p[1] + 1] = np.array([64, 217, 64])
            if p[0] + 1 < self.grid_size and  p[1] - 1 >= 0:
                grid[(p[0] + 1) * self.grid_size + p[1] - 1] = np.array([64, 217, 64])
            if p[0] - 1 >= 0 and p[1] + 1 < self.grid_size:
                grid[(p[0] - 1) * self.grid_size + p[1] + 1] = np.array([64, 217, 64])
            if p[0]  - 1 >= 0 and p[1] - 1 >= 0:
                grid[(p[0] - 1) * self.grid_size + p[1] - 1] = np.array([64, 217, 64])
            grid[p[0] * self.grid_size + p[1]] = np.array([0, 153, 0])
        for p in self._preys_pos:
            grid[p[0] * self.grid_size + p[1]] = np.array([204, 0, 0])
    ...
    def _take_preys_random_action(self):
        actions = np.random.choice(self.n_actions, size = self.n_preys)
        for a, p in zip(actions, self._preys_pos):
            if a == 1:
                p[0] = max(0, p[0] - 1)
            elif a == 2:
                p[0] = min(p[0] + 1, self.grid_size - 1)
    ...
    def _take_preys_selected_action(self, actions):
        for a, p in zip(actions, self._preys_pos):
            if a == 1:
                p[0] = max(0, p[0] - 1)
            elif a == 2:
    ...
    def _get_agents_reward(self):
        ...
        for p in self._preys_pos:
            ...
            for q in self._agents_pos:
                d = np.abs(np.subtract(p, q))
                min_d = min(min_d, np.sum(d))
                if np.all(np.less_equal(d, np.ones_like(p))):
                    reward += self.catch_reward
            if self.shaped_reward:
                reward -= self.shaped_reward_scale * min_d
        return reward
    ...
    def get_current_setting(self):
        env_setting = {"agents_pos": copy.deepcopy(self._agents_pos),
                        "preys_pos": copy.deepcopy(self._preys_pos),
    ...
    def set_current_setting(self, env_setting):
        ...
        self._preys_pos = copy.deepcopy(env_setting["preys_pos"])
    ...
    def get_heuristic(self, gamma):
        steps_per_prey = []
        for p in self._preys_pos:
            steps_per_prey.append([])
            for q in self._agents_pos:
                steps_per_prey[-1].append(np.abs(np.subtract(p, q)))
        heuristic = 0
        for i in range(self._n_steps, self.episode_limit):
            # WE USE -2 BECAUSE WE IMAGINE THAT THE AGENTS GO TOWARDS THE PREYS AND THE PREYS GO TOWARDS THE AGENTS
            steps_per_prey = [[np.array([max(0, j[0] - 2), max(0, j[1] - 2)]) for j in p] for p in steps_per_prey]
            for d in sum(steps_per_prey, []):
                if np.all(np.less_equal(d, np.ones_like(d))):
                    heuristic += (gamma ** i) * self.catch_reward
        return heuristic